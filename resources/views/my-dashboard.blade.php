                                ...
                                <tr style="background: {{ $order->status_bg ? $order->status_bg . '15' : 'transparent' }}; border-left: 4px solid {{ $order->status_bg ?? 'transparent' }}; transition: background 0.2s;">
                                    ...
                                    <td style="white-space: nowrap;">
                                        ...
                                        @if($order->lieferdatum)
                                            <div style="font-size: 0.7rem; color: {{ \Carbon\Carbon::parse($order->lieferdatum)->lt(now()->startOfDay()) ? '#ff4d4d' : 'var(--text-muted)' }}; margin-top: 2px; display: flex; align-items: center; gap: 4px; font-weight: {{ \Carbon\Carbon::parse($order->lieferdatum)->lt(now()->startOfDay()) ? '700' : '400' }};">
