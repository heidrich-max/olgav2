        ...
        <div class="card">
            ...
            <div class="status-nav">
                ...
                <a href="{{ route('orders.index', ['view' => $view, 'status' => $s->name, 'search' => $search, 'salesperson' => $selectedSalesperson]) }}" 
                   ...
                   style="{{ $selectedStatus == $s->name ? 'background-color: ' . $pColor . '; border-color: rgba(255,255,255,0.2);' : 'background-color: ' . ($isHex ? $pColor . '15' : 'rgba(255,255,255,0.05)') . '; border-color: ' . ($isHex ? $pColor . '88' : 'var(--glass-border)') . '; color: ' . $pColor . ';' }}"
                   title="{{ $s->name }}">
