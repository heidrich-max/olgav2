...
// Dashboard Controller with consistent revenue logic
...
class DashboardController extends Controller
{
    public function index(Request $request)
    {
        ...
        // 1. PROJECT REVENUE (Selected Month) - Live from auftrag_tabelle
        ...
        // 2. COMPANY REVENUE (Year Comparison) - Also live from auftrag_tabelle for CONSISTENCY
        $companyStats = new \stdClass();
        
        // Sum current year from live orders
        $companyStats->aktuell_jahr = OrderTable::where('firmen_id', $companyId)
            ->whereYear('erstelldatum', $selectedYear)
            ->sum('auftragssumme');

        // Note: Vorjahr still from auftrag_umsatz mapping, as we might not have full history in auftrag_tabelle
...
echo "DashboardController updated with consistent live revenue calculation.\n";
?>
